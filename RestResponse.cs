...
namespace RestSharp
	...
	public abstract class RestResponseBase
		...
		public string Content
			{
			get { return content ?? (content = RawBytes.AsString (ContentEncoding)); }
			set { content = value; }
			}
		...
		protected string DebuggerDisplay ()
			{
			return String.Format ("StatusCode: {0}, Content-Type: {1}, Content-Length: {2})", StatusCode, ContentType, ContentLength);
			}
		}
	...
	public class RestResponse : RestResponseBase, IRestResponse
		...
		private RestResponse SetCookies (IEnumerable<HttpCookie> cookies)
			{
			return this.With (
				x => x.Cookies = cookies.Select (c => RestResponseCookie.FromHttpCookie (c)).ToList ()
				);
			}
