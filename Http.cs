...
using DecompressionMethods = SSMono.Net.DecompressionMethods;
using HttpWebRequest = SSMono.Net.HttpWebRequest;
using HttpWebResponse = SSMono.Net.HttpWebResponse;
using ICredentials = SSMono.Net.ICredentials;
...
namespace RestSharp
	...
	public partial class Http : IHttp
		...
		internal Func<string, string> Encode
			{
		...
		private void AddSharedHeaderActions ()
			...
			restrictedHeaderActions.Add ("Date", (r, v) =>
				{
				DateTime parsed;
				if (DateTimeEx.TryParse (v, out parsed))
					r.Date = parsed;
				});
		...
		private string EncodeParameters ()
			{
			return
				StringEx.Join ("&", Parameters.Select (p => String.Format ("{0}={1}", Encode (p.Name), Encode (p.Value))));
			}
