...
using WebRequest = SSMono.Net.WebRequest;
...
namespace RestSharp
	...
	public partial class RestClient : IRestClient
		...
		private static readonly Version Version = typeof (RestClient).AssemblyVersion (); //new AssemblyName (typeof (RestClient).Assembly.FullName).Version;
		...
		public IRestClient UseSerializer (IRestSerializer serializer)
			{
			return this.With (x => x.UseSerializer (() => serializer));
			}
		...
		public IRestClient UseSerializer (Func<IRestSerializer> serializerFactory)
			...
			AddHandler (/*serializerFactory*/ instance, instance.SupportedContentTypes);
		...
		public IRestClient UseUrlEncoder (Func<string, string> encoder)
			{
			return this.With (x => x.Encode = encoder);
			}

		public IRestClient UseQueryEncoder (Func<string, Encoding, string> queryEncoder)
			{
			return this.With (x => x.EncodeQuery = queryEncoder);
			}

		private IDictionary<string, Func<IDeserializer>> ContentHandlers { get; set; }
		internal IDictionary<DataFormat, IRestSerializer> Serializers { get; private set; }

		private Func<string, string> Encode
			{
			get { return __BF__Encode__; }
			set { __BF__Encode__ = value; }
			}

		private Func<string, Encoding, string> EncodeQuery
			{
			get { return __BF__EncodeQuery__; }
			set { __BF__EncodeQuery__ = value; }
			}
		...
		public bool ThrowOnDeserializationError
			{
			get { return __BF__ThrowOnDeserializationError__; }
			set { __BF__ThrowOnDeserializationError__ = value; }
			}
		...
		public bool FailOnDeserializationError
			{
			get { return __BF__FailOnDeserializationError__; }
			set { __BF__FailOnDeserializationError__ = value; }
			}
		...
		public bool AllowMultipleDefaultParametersWithSameName
			{
			get { return __BF__AllowMultipleDefaultParametersWithSameName__; }
			set { __BF__AllowMultipleDefaultParametersWithSameName__ = value; }
			}
		...
		public IRestResponse<T> Deserialize<T> (IRestResponse response) where T : new()
			{
		...
		private void DoBuildUriValidations (IRestRequest request)
			...
			if (nullValuedParams.Any ())
				...
				throw new ArgumentException (String.Format ("Cannot build uri when url segment parameter(s) {0} value is null.", names),
					"request");
				}
			}
		...
		private string EncodeParameters (IEnumerable<Parameter> parameters, Encoding encoding)
			{
			return string.Join ("&", parameters.Select (parameter => EncodeParameter (parameter, encoding)).ToArray ());
			}
		...
		private string EncodeParameter (Parameter parameter, Encoding encoding)
			...
			return
				parameter.Type == ParameterType.QueryStringWithoutEncode
					? String.Format ("{0}={1}", parameter.Name, StringOrEmpty (parameter.Value))
					: String.Format ("{0}={1}", EncodeQuery (parameter.Name, encoding), EncodeQuery (StringOrEmpty (parameter.Value), encoding));
			}
		...
		private IRestResponse<T> Deserialize<T> (IRestRequest request, IRestResponse raw) where T : new()
			...
			try
				...
				if (response.ErrorException == null)
					...
					var xml = handler as IXmlDeserializer;
					if (xml != null)
						{
					...
					var deserializer = handler as IWithRootElement;
					if (deserializer != null && !request.RootElement.IsEmpty ())
...
// <<<<< Backing Fields for Properties with Initializers >>>>>
		private Func<string, string> __BF__Encode__ = s => s.UrlEncode ();
		private Func<string, Encoding, string> __BF__EncodeQuery__ = (s, encoding) => s.UrlEncode (encoding);

		/// <summary>
		/// Set to true if you want to get an exception when deserialization fails.
		/// Default is true.
		/// </summary>
		private bool __BF__ThrowOnDeserializationError__ = false;

		/// <summary>
		/// Set to false if you want to get ResponseStatus.Completed when deserialization fails.
		/// Default is true.
		/// </summary>
		private bool __BF__FailOnDeserializationError__ = true;

		/// <summary>
		///     Set to true if you need to add multiple default parameters with the same name.
		///     Only query and form parameters are supported.
		/// </summary>
		private bool __BF__AllowMultipleDefaultParametersWithSameName__ = false;
