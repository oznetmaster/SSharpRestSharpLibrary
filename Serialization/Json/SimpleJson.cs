	...
	public
				...
				if (c == '"')
					...
					if (c == '"')
						...
						if (remainingLength >= 4)
							...
							uint codePoint;
							if (!(success = UInt32Ex.TryParse (new string (json, index, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out codePoint)))
								return "";
							...
							if (0xD800 <= codePoint && codePoint <= 0xDBFF) // if high surrogate
								...
								if (remainingLength >= 6)
									...
									if (new string (json, index, 2) == "\\u" &&
									    UInt32Ex.TryParse (new string (json, index + 2, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out lowCodePoint))
										{
		...
		private static object ParseNumber (char[] json, ref int index, ref bool success)
			...
			if (str.IndexOf (".", StringComparison.OrdinalIgnoreCase) != -1 || str.IndexOf ("e", StringComparison.OrdinalIgnoreCase) != -1)
				{
				double number;
				success = DoubleEx.TryParse (new string (json, index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture, out number);
				returnNumber = number;
				}
			else
				{
				long number;
				success = Int64Ex.TryParse (new string (json, index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture, out number);
				returnNumber = number;
				}
			index = lastIndex + 1;
			return returnNumber;
...
#else
				if (info == null || type == null || !CAttribute.IsDefined (info, type))
					return null;
				return CAttribute.GetCustomAttribute (info, type);
#endif
				}
...
#else
				return type.GetCType ().GetConstructors ();
#endif
				}
